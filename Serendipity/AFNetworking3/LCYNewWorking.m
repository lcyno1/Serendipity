...
+(void)GetDataWithURL:(NSString *)urlStr dic:(NSDictionary *)dic success:(void(^)(id responseObject))response failed:(void(^)(NSError *error))err{
    ...
    //只支持Xcode7.0以上版本 AFNetworking3.0以上版本
   ...
   [man.responseSerializer setAcceptableContentTypes:[NSSet setWithObjects:@"text/html", @"application/json", @"text/json", @"text/javascript",@"text/html",@"text/css", @"text/plain", @"application/x-javascript", @"application/javascript",nil]];
    ...
    [man GET:urlStr parameters:dic success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        ...
        //拿到缓存路径
        //response后面参数,因为我们没走网络请求,不是缓存数据而是获取数据
...
+(void)PostDataWithURL:(NSString *)urlStr dic:(NSDictionary *)dic success:(void (^)(id))response failed:(void (^)(NSError *))err{
